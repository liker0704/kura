...
Please report vulnerabilities privately through [GitHub Security Advisories](https://github.com/jayminwest/kura/security/advisories).

1. Go to the [Security Advisories page](https://github.com/jayminwest/kura/security/advisories)
...
Kura is a CLI tool that reads and writes files on the local filesystem. The following are considered security issues:

- **Command injection** -- Unsanitized input passed to shell execution
- **Path traversal** -- Accessing files outside the intended `.kura/` directory
...
- Denial of service via large input files (Kura is a local tool, not a service)
- Issues that require the attacker to already have local shell access with the same privileges as the user
- Social engineering or phishing

## Security Measures

Kura already implements several hardening measures:
